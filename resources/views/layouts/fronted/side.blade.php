<ul class="text-gray-100 grid grid-flow-col bg-white px-4 lg:grid-cols-1 place-content-between py-8">
    @if($cats->count())
    @foreach($cats as $cat)
    <li>
        <a href="{{route('prod.allcateg', [$cat->id,$cat->slug_en])}}">
            <div class="text-green-900 hover:text-green-100 font-bold text-right w-full px-2 py-3 rounded-t" style="background: linear-gradient(45deg, white 0 25%, rgba(52, 211, 153, 0.8) 25% 100%)">
                {{session()->get('language') == 'hebrew' ? $cat->name_heb : $cat->name_en}}
    ...
    <div>
        ...
        <li>
            <a href="#">
                <div class="text-blue-900 hover:text-blue-100 font-bold px-2 py-3 rounded mb-4 text-right {{session()->get('language') == 'hebrew' ? 'rtl' : ''}}" style="background: linear-gradient(45deg, white 0 25%, rgba(25, 181, 254, 0.8) 25% 100%)">{{session()->get('language') == 'hebrew' ? 'חדש' : 'New Arivals'}}
        ...
        <li>
            <a href="{{url('/contact')}}">
                <div class="text-purple-900 hover:text-purple-100 font-bold px-2 py-3 rounded mb-4 text-right {{session()->get('language') == 'hebrew' ? 'rtl' : ''}}" style="background: linear-gradient(45deg, white 0 25%, rgba(150, 25, 254, 0.8) 25% 100%)">{{session()->get('language') == 'hebrew' ? 'צור קשר' : 'Contact Us'}}
