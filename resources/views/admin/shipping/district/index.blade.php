...
<div class="p-4 bg-gray-900 md:rounded">
    ...
    <div class="p-4 md:rounded bg-gray-800 mb-2">
        <h3 class="font-serif font-semibold text-lg text-gray-100 leading-tight">
            All Shipping Area Districts Information
        </h3>
        <p class="text-sm text-gray-300 leading-tight mt-1 mb-2">
            Veiw and Manage All Districts.
        </p>
    </div>
    <!-- table -->
                ...
                <div>
                    <table class="w-full text-sm">
                        ...
                        <tbody>
                            ...
                            <tr class="font-light">
                                ...
                                <td class="rounded-lg bg-gray-800 border-2 border-gray-700">
                                    <div class="flex justify-around content-center">
                                        @auth('admin')
                                        <form action="{{route('admin.shipping.district.edit', $item->id)}}" method="GET" class="px-1 my-2">
                                        ...
                                        <form action="{{route('admin.shipping.district.delete', $item->id)}}" method="POST" class="px-1 my-2">
                                            @method('DELETE')
                                            @csrf
                            ...
                            @else
                            <tr>
                                <td colspan="4" class="font-bold text-center text-gray-100">
                                    <div>Wh..ops <br> There are no districts yet...</div>
                                </td>
                            </tr>
                            @endif
                        </tbody>
    ...
    <div class="p-4 md:rounded bg-gray-800 mb-2">
        ...
        <p class="text-sm text-gray-300 leading-tight mt-1 mb-2">
            Add new Shipping Area District.
        </p>
    </div>
    <div class="p-4 md:p-6 lg:py-10 lg:px-12 md:rounded bg-gray-700">
        <form action="{{route('admin.store.shipping.district')}}" method="POST" class="w-full">
            @csrf
            <div class="mb-2 grid grid-cols-6 lg:gap-4">
                ...
                <input type="text" name="name" id="name" class="col-span-6 lg:col-span-4 rounded-lg text-sm border @error('name')  border-red-500 @enderror" value="{{old('name')}}" placeholder="district name" required autofocus>
            </div>
            @error('name')
